...
using X4_ComplexCalculator.Common.Controls.DataGridFilters.Interface;
using X4_ComplexCalculator_CustomControlLibrary.DataGridExtensions;


namespace X4_ComplexCalculator.Common.Controls.DataGridFilters.MultiList;
...
public sealed partial class MultiListFilter
{
    ...
    public override void OnApplyTemplate()
    {
        ...
        if (_filterColumnControl?.LoadFilter() is MultiListContentFilter prevFilter)
        {
            ...
            {
                item.IsChecked = !prevFilter.ExcludedItems.Contains(item.Text);
            }
        }
        else
        {
    ...
    public static readonly DependencyProperty FilterProperty =
        DependencyProperty.Register(
            ...
            typeof(MultiListFilter),
            new FrameworkPropertyMetadata(new MultiListContentFilter(Enumerable.Empty<string>()), FrameworkPropertyMetadataOptions.BindsTwoWayByDefault)
        );
    ...
    public static readonly DependencyProperty ListBoxSearchTextProperty =
        DependencyProperty.Register(
            ...
            new FrameworkPropertyMetadata(
                "",
                FrameworkPropertyMetadataOptions.BindsTwoWayByDefault,
                (sender, e) => ((MultiListFilter)sender).CheckListBoxSearchText_Changed()
            )
        );
    ...
    {
        Filter = new MultiListContentFilter(ListBoxItems.Where(x => !x.IsChecked).Select(x => x.Text));
        IsOpen = false;
    }
