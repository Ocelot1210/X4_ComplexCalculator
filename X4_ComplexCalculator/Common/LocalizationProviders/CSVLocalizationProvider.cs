...
namespace X4_ComplexCalculator.Common.LocalizationProviders;
...
public class CSVLocalizationProvider : FrameworkElement, ILocalizationProvider
{
    ...
    public string FileName
    {
        ...
        set
        {
            if (_fileName != value)
            {
                ...
                {
                    var csv = Path.Combine(appPath, "Localizations", FileName + (string.IsNullOrEmpty(c.Name) ? "" : $".{c.Name}") + ".txt");
                    if (File.Exists(csv))
    ...
    public object GetLocalizedObject(string key, DependencyObject target, CultureInfo culture)
    {
        ...
        const string CSV_DIRECTORY = "Localizations";
        var csvPath = "";
        ...
        {
            csvPath = Path.Combine(GetWorkingDirectory(), CSV_DIRECTORY, dictionary + (string.IsNullOrEmpty(culture.Name) ? "" : "." + culture.Name) + ".txt");
        ...
        if (!File.Exists(csvPath))
        {
            ...
            csvPath = Path.Combine(GetWorkingDirectory(), CSV_DIRECTORY, dictionary + ".txt");
