...
namespace X4_ComplexCalculator.Main.WorkArea.StationSummary.WorkForce.NeedWareInfo
{
    ...
    /// 必要ウェア情報
    /// </summary>
    class NeedWareInfoModel : INotifyPropertyChangedBace, IDisposable
    {
        #region メンバ
        /// <summary>
        /// 製品情報
        ...
        /// </remarks>
        private Dictionary<string, long> AggregateTargetProducts = new Dictionary<string, long>();
        #endregion
...
SELECT
	...
	Ware.WareID,
	Ware.Name,
	CAST(((CAST({NeedWorkforce} AS REAL) / WorkUnitProduction.Amount) * WorkUnitResource.Amount + 0.5) AS INTEGER) AS Amount
...
WHERE
	...
	WorkUnitResource.WareID       = Ware.WareID";
        ...
        private void Products_CollectionPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            {
                AggregateTargetProducts[wareID] = product.Count;

                // 現在の列にあればそちらを使用
                var itm = NeedWareInfoDetails.Where(x => x.WareID == wareID).FirstOrDefault();
                if (itm != null)
                {
                    itm.ProductionAmount = product.Count;
                }
            }
        }
        ...
        private void Products_CollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            ...
            foreach (var item in items)
            {
                ...
                {
                    AggregateTargetProducts[prod.Ware.WareID] = prod.Count;

                    var ware = NeedWareInfoDetails.Where(x => x.WareID == prod.Ware.WareID).FirstOrDefault();
                    if (ware != null)
                    {
                        ware.ProductionAmount = prod.Count;
                    }
                }
            }
        }
