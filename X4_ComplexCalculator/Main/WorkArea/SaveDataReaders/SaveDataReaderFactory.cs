...
internal static class SaveDataReaderFactory
{
    ...
    public static ISaveDataReader CreateSaveDataReader(string path, IMessenger messenger, IWorkArea workArea)
    {
        var version = GetVersion(path);
        ISaveDataReader ret = version switch
        {
            0 => new SaveDataReader0(messenger, workArea),
            1 => new SaveDataReader1(messenger, workArea),
            _ => new SaveDataReader2(messenger, workArea),
        };
